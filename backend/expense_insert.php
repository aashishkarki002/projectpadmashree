...
if (!$conn) {<?php 
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {     
        ...
        $check_category = "SELECT id FROM expense_categories WHERE category_name = ?";
        $stmt = mysqli_prepare($conn, $check_category);
        ...
        try {
            ...
            mysqli_stmt_bind_param($stmt, "iidss", $user_id, $category_id, $amount, $date, $notes);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $amount = floatval($_POST['amount']);
    $date = mysqli_real_escape_string($conn, $_POST['date']);
    $notes = mysqli_real_escape_string($conn, $_POST['note'] ?? ''); // Changed to match form field name
    ...
    try {
        $query = "INSERT INTO expense (u_id, category_id,  amount, date, notes) VALUES (?, ?,  ?, ?, ?)";
        $stmt = mysqli_prepare($conn, $query);
        mysqli_stmt_bind_param($stmt, "iisdss", $user_id, $category_id, $category, $amount, $date, $notes);
